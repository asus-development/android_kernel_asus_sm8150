...
//[+++]Add back this from Android Q to Android R
	...
	case TAPER_CHARGE:
		if (g_fgChip != NULL && smbchg_dev != NULL) {
			if (asus_get_prop_batt_capacity(smbchg_dev) == 100) {
				...
				//[+++] Porting for COS. Don't send extra battery status to healthd
				#if 0
				if (g_Charger_mode) {
...
void set_qc_stat(union power_supply_propval *val)
{
	...
	//ASUS_BSP : Porting for COS. Support to send extcon in COS
	#if 0
	if (g_Charger_mode)
		return;
	#endif
